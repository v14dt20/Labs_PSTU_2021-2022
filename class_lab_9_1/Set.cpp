...
Set::Set(int s)
{
	if (s > MAX_SIZE) throw 1;//если рзамер множества больше максимального, генерируетс€ исключение
	size = s;
	beg = new int[size];
...
{
	if (index < 0) throw 2;//если обращаемс€ к индексу меньше 0, генерируетс€ исключение
	if (index >= size) throw 3;//если обращаемс€ к индесу больше текущего размера множества, генерируетс€ исключение
	return beg[index];
}
...
Set Set::operator++()
{
	if (size + 1 == MAX_SIZE) throw 4;//если при добавлении элемнта размер множества больше макс, генерируетс€ исключение
